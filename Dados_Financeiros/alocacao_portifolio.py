...
class Alocacao_Otimizacao(object):
    ...
    def alocacao_portifolio(self):
        ...
        print()
        a = Alocacao_Ativos(pd.read_csv('acoesGerais.csv'), self.dinheiro_total, 1, melhores_pesos = melhores_pesos)
        acoes_pesos, soma_valor,_,_,_,_,_ = a.alocacao_ativos()

        print(acoes_pesos,'\n')
        print('soma_valor',soma_valor)
        
        plt.figure()
        plt.scatter(Lista_volatividade_esperada,Lista_retorno_esperado, c = Lista_sharpe_ratio)
